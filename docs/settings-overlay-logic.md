...
The app-level General tab includes a Show Preview control for Practice, Qualifying, and Race. Preview mode swaps the live telemetry source to deterministic session fixtures so native overlays and localhost browser sources render realistic mock session data. It does not override overlay `Enabled` state, session filters, positioning, scale, opacity, Stream Chat configuration, or topmost behavior; hidden overlays stay hidden unless the user has enabled them or is using an existing overlay-specific settings preview.

Standings and Relative use a shared content-column manager. The content definition has reusable data keys, but each overlay owns its own option keys so changing Standings columns does not imply Relative changes. Columns remain in one ordered list even when disabled, are dimmed when hidden, expose pixel widths, and can be reordered. Native and browser rendering use the same compact default content and the recommended OBS width follows visible column widths instead of being floored by the old large default overlay width.
...
The Design V2 settings surface does not show static render-preview panels for ordinary overlay tabs. Those panels are hidden until a real preview renderer exists so they do not imply live overlay rendering where none is wired. Garage Cover keeps its image preview and test-cover action because that tab previews the actual configured browser-source cover image. Radar keeps its native settings-preview behavior through the live overlay window rather than an embedded settings-tab render.
