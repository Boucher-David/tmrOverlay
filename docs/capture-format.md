...
- `live-overlay-diagnostics.json` summarizes passive gap/radar/fuel/position-cadence/lap-delta/sector-timing assumptions and track-map sector-highlight coverage when overlay diagnostics collection is enabled, including local-only radar suppression, missing lap-counter fallback, reset-style progress discontinuities, and partial/degraded radar signals.
