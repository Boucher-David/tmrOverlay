...
- `live-overlay-diagnostics.json` summarizes passive gap/radar/fuel/position-cadence/lap-delta/sector-timing assumptions and track-map sector-highlight coverage when overlay diagnostics collection is enabled, including local-only radar suppression and partial/degraded radar signals.
