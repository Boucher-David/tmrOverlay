...
While model-v2 parity data is still being collected, browser review is the primary local parity loop and screenshot generator for native plus localhost behavior. The tracked mac harness is deprecated secondary scaffolding for legacy `mocks/design-v2/` contact sheets and mock live scenes. New design iteration should start in browser review, then use Windows/native validation for production behavior.
