...
The settings tab lets the user choose which flag categories can display and set the custom overlay size. There are no per-flag display timers: each flag paints while the current telemetry state maps to an enabled user-facing flag category and clears when that telemetry state clears or changes to a disabled category. Multiple meaningful categories can render at once, so persistent states such as white, meatball, and black can stay up while transient yellow/debris/blue states appear or disappear around them.
