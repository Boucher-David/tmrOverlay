...
Scoring rows are grouped by class in the official multiclass table order from iRacing, not reordered around the focused/reference car class. A separate class-separator content block controls whether class separators are shown and how many rows from each other class are kept visible when the row budget is tight; the focused class still receives enough row budget to keep the reference context visible. Class separators render as full-width class bands instead of ordinary data rows with per-column cells. They use the iRacing class color and can show class-specific race projection context when a timed race has enough rolling lap evidence. The native overlay and localhost browser source both honor the same settings. Cars present in the scoring snapshot remain visible even when live timing coverage is partial, so iRacing proximity/render limits cannot compress missing cars out of the standings. Rows with no live timing enrichment are muted and show unavailable live timing cells rather than disappearing.
