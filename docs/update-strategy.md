...
The public GitHub Release is the Velopack feed. Installed builds use Velopack `GithubSource` against `https://github.com/Boucher-David/TMROverlay` with no embedded token. The active pre-1.0 package id is `TMROverlay`; it intentionally replaces the first tester id, `TechMatesRacing.TmrOverlay`, so installed identity uses the shorter app name. The MSI uses branded WiX banner/logo images and creates a Start Menu shortcut instead of Velopack's default desktop shortcut. Portable/dev runs skip update checks because they do not have a Velopack install identity.
