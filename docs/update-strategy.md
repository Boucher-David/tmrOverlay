...
The v0.9 publishing branch defines a tag-driven portable Windows release path. A `vMAJOR.MINOR.PATCH` tag runs the Windows build/test gate, publishes the app as a self-contained `win-x64` package, audits the publish folder, zips it, writes a package manifest and SHA-256 checksum, uploads workflow artifacts, and attaches the zip/manifest/checksum to a GitHub Release.
...
1. Keep the PR/main Windows build-test workflow as the merge gate. Implemented in `.github/workflows/windows-dotnet.yml`.
