...
| Radar focus handling | Radar did not handle focused cars well and struggled when the user's team car was driven by a teammate. | Matches the raw/IBT split: live `CarIdx*` arrays are needed for focus/radar, while scalar player-only signals like `CarLeftRight` do not apply to arbitrary focus cars. | Model v2 should separate focus-car placement, team-car placement, local-player side occupancy, and teammate-driver context. Radar should not imply side occupancy for non-local focus. |
...
| Gap graph update cadence | The graph probably needs to update more often than lap completion, and it is worth testing whether iRacing exposes live position changes before lap completion. | The collector already reads `CarIdxPosition` and `CarIdxClassPosition` each frame, and the raw/offline analysis plus live overlay diagnostics now count sampled intra-lap position/class-position changes. | Use the new cadence evidence across several sessions before treating continuous standings/relative updates as a model-v2 guarantee. |
| Multiclass warning | Radar multiclass warning did not work well, likely due to the same focus/team-driver source problems as radar placement. | Matches current evidence: side occupancy is local-player scalar, while focus-relative placement depends on valid focused `CarIdx*` timing/progress. | Multiclass warning should be rebuilt on top of model-v2 focus-relative timing evidence and should suppress or relabel partial/non-focus-safe states. |

## Recommended Follow-Up

1. Use `live-overlay-diagnostics.json` and `tools/analysis/analyze_capture_assumptions.py` outputs to gather more intra-lap position/class-position evidence.
...
4. Rebuild radar focus handling around model-v2 evidence before expanding multiclass warning behavior.
