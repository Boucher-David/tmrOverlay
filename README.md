...
- Windows-rendered screenshots are generated by CI from real WinForms forms and uploaded under `windows-overlay-screenshots`; installer menu screenshots are uploaded under `windows-installer-screenshots`. CI also publishes a `windows-screenshots` bundle with both sets for side-by-side collection.
