...
- Shows first-pass standings, relative, track-map, stream-chat, flags, session/weather, pit-service, input/car-state, radar, and gap-to-leader native overlays backed by model-v2 timing/relative/session/weather/fuel-pit/input/spatial/race-event/track-map rows plus live `CamCarIdx`, `CarLeftRight`, `CarIdxF2Time`, `CarIdxEstTime`, and `CarIdx*` progress/position telemetry. Race-data overlays fade out when live telemetry is unavailable instead of leaving stale race state on screen. Stream Chat can use a saved public Twitch channel in the native overlay, and the localhost browser-source route can use either a saved Streamlabs Chat Box widget URL or saved public Twitch channel chat.
  ...
  The track-map overlay is a transparent map-only surface: it draws bundled app maps when available, uses local IBT-derived maps while default-on generation remains enabled, otherwise shows a circle fallback with smoothed live car dots placed by lap-distance progress. The focused/player marker can show its current `P<N>` position inside the dot, schema-v2 map assets include sector boundaries, and `LiveTelemetrySnapshot.Models.TrackMap` overpaints only live green/purple sector status on top of the continuous white line. The Track Map opacity setting adjusts only the map's internal fill while the track outline stays fully opaque.
  The garage-cover route is a localhost-only streamer privacy surface for OBS: it watches iRacing's Garage-visible state, then shows an opaque user-imported image or black TMR fallback in the browser source so setup details are hidden from the stream. The app does not create a desktop Garage Cover window. The flags overlay is a compact transparent native overlay driven by live session flags; it draws icon-only procedural green, blue, yellow/caution, red, black, meatball, white, and checkered flags, can show multiple active categories at once, ignores background-only flag bits by themselves, and suppresses plain steady-state green running. Session/weather and pit-service use the shared simple telemetry shell; session/weather now includes current precipitation, wind, humidity/fog, and short condition-change highlights while keeping forecast display disabled unless a future telemetry schema exposes reliable forecast data. Input/car-state uses graph traces, and all render direct iRacing telemetry quietly. Pit-service is read-only but now includes a red/green release row from pit service completion telemetry, with command-capable pit crew/engineer workflow deferred to a future analysis/operator overlay.
...
- When raw capture is enabled, stores `telemetry.bin`, `telemetry-schema.json`, `latest-session.yaml`, optional `session-info/`, and `capture-manifest.json`, then writes compact `capture-synthesis.json` plus default-on advanced sidecars such as `live-model-parity.json`, `live-overlay-diagnostics.json`, and `ibt-analysis/*.json` when those collectors emit for the session. Successful IBT analysis can also generate a local reusable track-map JSON under app-owned storage unless the user opts out on the Track Map settings tab.
- Keeps compact edge-case telemetry, model-v2 parity, live-overlay diagnostics, post-race analysis, and localhost browser-source diagnostics enabled by default while raw capture remains opt-in; teammate-facing Support stays focused on version details, diagnostic capture, diagnostics bundles, current state, and support folders.
...
- `src/TmrOverlay.App/Localhost/` contains the optional localhost HTTP server for OBS and other local capture tools; overlay-specific browser-source pages live with the overlay modules.
...
Each `*-live-overlay-diagnostics.json` file summarizes current-overlay and design-v2 candidate assumptions observed from normalized live snapshots: gap source/session semantics, large gap and jump examples, local-only radar suppression, radar side/focus/placement evidence, fuel level/burn/source evidence, sampled intra-lap position/class-position changes, live lap-delta channel availability, derived sector-timing coverage from sector metadata plus car progress, lap-counter gaps, reset-style progress discontinuities, and track-map model-v2 sector highlight coverage. Event examples are duplicate-suppressed and capped per kind so one stable condition cannot fill the whole sample budget. Performance diagnostics also record live-telemetry availability and fade alpha for race-data overlays. The mac harness mirrors track-map highlight diagnostics for mock/demo overlay runs.
...
The optional localhost overlay server is enabled by default so local OBS/browser-source routes and request diagnostics are available without an app restart. Override it only when the local HTTP server should stay off:

```powershell
$env:TMR_LocalhostOverlays__Enabled = "false"
...
The localhost pages poll normalized `ILiveTelemetrySource` snapshots and do not talk to iRacing directly. Overlay modules own their browser-source route metadata and page scripts; localhost owns only the HTTP transport and generic shell. Track Map fetches stored map geometry from `GET /api/track-map` separately from live snapshots. Garage Cover fetches imported image metadata from `GET /api/garage-cover` and the app-owned imported image from `GET /api/garage-cover/image`. The app records localhost lifecycle state, request counters, route counts, and failures into diagnostics bundles so support can tell whether the service was disabled, listening, requested, or failing. Flags intentionally has no browser-source route yet while the native overlay remains Windows-only. Stream Chat reads one selected saved source at a time from the Stream Chat settings tab: either a Streamlabs Chat Box widget URL embedded in the browser source, or public Twitch channel chat by channel name. The native Stream Chat overlay auto-connects to the saved Twitch channel when enabled; Streamlabs remains browser-source only. Both chat surfaces replace the first placeholder row with a connected or connection-error message. Provider auth, moderation, and write/chat-command support are intentionally separate follow-up work. Streamlabs widget URLs are treated as private local settings and are redacted from diagnostics bundles. Overlay Bridge is reserved for a future teammate-to-teammate data-sharing boundary, not this local OBS/browser-source feature.
...
Performance diagnostics are always on, even when raw capture is disabled. The app writes periodic JSONL snapshots under `%LOCALAPPDATA%\TmrOverlay\logs\performance` with telemetry throughput, iRacing network/system values such as channel quality, latency, frame rate, CPU/GPU use, replay/on-track state, overlay refresh timing including settings-window refresh/sync timings, overlay update-decision counters, capture writer state when available, process memory, and GC counts.

Bundles include app/storage metadata, telemetry state, localhost overlay state/request counters, track-map inventory metadata, lightweight performance snapshots, recent performance logs, runtime state, settings, recent logs/events, latest capture metadata and compact sidecars, recent user-history summaries/aggregates for car/track/session accuracy checks, and advanced collection artifacts such as edge-case telemetry, model-v2 parity, overlay diagnostics, or post-race analysis when those files exist. Edge-case telemetry watches reset/tow context such as `EnterExitReset` and `PlayerCarTowTime` when those SDK channels are present. Bundles intentionally exclude raw `telemetry.bin` and source `.ibt` payloads.
...
```powershell
python tools/validate_overlay_screenshots.py --profile windows-expectations
...
The expectations check catches stale expected dimensions after overlay default-size changes. The Windows PNGs are uploaded as workflow artifacts for parity review instead of being committed under `mocks/`.
