...
The production app is Windows-only because it talks to iRacing through the SDK. The repository also contains an ignored macOS harness for mock-telemetry development and screenshot iteration.
...
- **Settings app:** a fixed-size WinForms control surface for overlay visibility, scale, opacity where useful, session filters, units, update state, support actions, and diagnostics.
...
- **Telemetry model:** normalized live snapshot models for timing, relative, spatial/radar, weather, fuel/pit, race events, inputs, and track-map state.
...
Those snapshots include telemetry throughput, iRacing network/system values, overlay refresh timings, timer cadence and active-count summaries, lifecycle visibility/fade states, skipped unchanged-sequence updates, paint samples, localhost activity, process memory, GDI/USER handle counts, and GC counts.
