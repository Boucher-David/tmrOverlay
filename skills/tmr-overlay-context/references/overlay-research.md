...
- radar: transparent 300px circular proximity view; scalar side occupancy from `CarLeftRight`, plus fresh live nearby car rectangles from `CarIdxEstTime`, `CarIdxF2Time`, and `CarIdx*` progress. Car rectangles color from white to yellow to red as traffic gets closer, and the overlay itself fades in/out. Faster approaching multiclass traffic can show a short outer warning arc with a live seconds gap before it reaches close radar range.
