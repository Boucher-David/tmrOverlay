...
- `src/TmrOverlay.App/Telemetry/TelemetryCaptureHostedService.cs`
  ...
  - records passive live overlay diagnostics in observer mode; `live-overlay-diagnostics.json` summarizes gap session semantics, large gap/jump evidence, local-only radar suppression, side/focus/placement evidence, fuel source evidence, sampled position cadence, lap-delta channel availability, derived sector-timing coverage, and track-map sector-highlight coverage
...
- `src/TmrOverlay.Core/Telemetry/Live/`
  ...
  - `LiveTelemetrySnapshot.Models` is an additive live-only model layer with session, driver-directory, timing, relative, spatial, track-map, weather, fuel/pit, race-event, and input availability families; it is derived from existing normalized samples and keeps raw capture, compact history, post-race analysis, and existing synthesized data backwards-compatible
...
- `src/TmrOverlay.App/Localhost/`
  ...
  - exposes `GET /health`, `GET /snapshot`, `GET /api/snapshot`, `GET /api/track-map`, `GET /api/stream-chat`, `GET /api/garage-cover`, `GET /api/garage-cover/image`, and per-overlay HTML routes under `/overlays/{id}`
...
- v0.9 portable tester publishing is tag-driven, but broad production distribution still needs a signing decision and installer/update channel. v0.12 should prefer Velopack-compatible release metadata for passive once-per-startup update checks, with a manifest/GitHub Release fallback only while portable zip releases remain the active channel.
