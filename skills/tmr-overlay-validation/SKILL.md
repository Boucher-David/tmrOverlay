...
- For overlay or settings UI changes, regenerate browser review screenshots with `npm run screenshots:browser -- --output artifacts/browser-review-screenshots` and inspect the uploaded `browser-review-screenshots` artifact from CI. The browser review artifact is the local screenshot authority because browser routes are expected to match localhost, with native Windows still validated separately.
