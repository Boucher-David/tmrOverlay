...
The settings window is a normal desktop window, not an always-on-top driving overlay. It uses a centered fixed-size 1080x600 panel with a TMR logo plus `Tech Mates Racing Overlay` header, vertical left-side tabs, General first, user-facing overlays next, and Support last. Overlay tabs expose opacity where it applies; flags and radar omit it. The radar tab previews the radar overlay only when that overlay is enabled.
